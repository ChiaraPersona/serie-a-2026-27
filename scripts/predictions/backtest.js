...
function predict(history, match, parameters, baselineOnly = false) {
  ...
  const homeLambda = clamp(league.home * homeAttack * awayDefence, 0.3, 3.5);
  const awayLambda = clamp(league.away * awayAttack * homeDefence, 0.25, 3.3);
  const rows = matrix(homeLambda, awayLambda, scoreCalibration(history, league, parameters.calibrationExponent));
  return { homeLambda, awayLambda, rows, probabilities: oneXTwo(rows) };
}
...
function evaluate(parameters, fromMatchday, toMatchday, baselineOnly = false) {
  const rows = matches.filter(match => match.matchday >= fromMatchday && match.matchday <= toMatchday).map(match => {
    ...
    const prediction = predict(history, match, parameters, baselineOnly);
...
const output = {
  ...
  headToHead: {
    ...
    note: "Il confronto con e senza ultimi cinque scontri diretti verra eseguito quando il dataset H2H sara disponibile nel repository."
  },
  caveats: [
