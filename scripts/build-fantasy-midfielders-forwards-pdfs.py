...
def load_indexes():
    ...
    return {
        "quotations": quotations["players"],
        "advice": {player["id"]: player for player in advice["players"]},
...
def build_rows(role, indexes):
    rows = []
    for quote in (player for player in indexes["quotations"] if player["role"] == role):
        player_id = quote.get("playerId")
        player = indexes["advice"].get(player_id)
        fantasy = indexes["fantasy"].get(str(quote["sourceId"]))
        ...
        rows.append({
            ...
            "minutes": minutes,
            "goalsAssists": f"{text_value(goals)} / {text_value(assists)}" if finite(goals) and finite(assists) else "N/D",
...
def build_pdf(role, indexes):
    config = ROLES[role]
    rows = build_rows(role, indexes)
    covered = sum(row["goalsAssists"] != "N/D" for row in rows)
    output = ROOT / "output" / "pdf" / f"fantacalcio-listone-consigli-{config['slug']}.pdf"
    ...
    document = SimpleDocTemplate(
        ...
        topMargin=13 * mm,
        bottomMargin=15 * mm,
        title=f"Fantacalcio - Listone e consigli {config['slug']}",
        author="Serie A 2026/27",
        subject=f"Listone {config['slug']} con statistiche 2025/26",
    )
    ...
    heading = [
        ...
        [Paragraph("LISTONE E CONSIGLI", ParagraphStyle("Kicker", parent=body_bold, fontSize=8, textColor=BLUE, leading=10)), Paragraph(config["title"], title_style)],
    ...
    headers = ["#", "Stelle", config["singular"], "Squadra", "Indice", "Qt.", "FVM", "Val. mercato", "Pres.", "Minuti", "G / A", "Cartellini"]
    ...
    print(f"Creato {output} ({len(rows)} {config['slug']}, {covered} con G/A disponibili).")
    return output, len(rows), covered


def main():
    indexes = load_indexes()
    for role in ("C", "A"):
        build_pdf(role, indexes)
