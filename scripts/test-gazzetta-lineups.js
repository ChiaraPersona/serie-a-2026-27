...
const dataset = JSON.parse(fs.readFileSync(path.join(root, "data/sources/gazzetta-probable-lineups-md1-2026-27.json"), "utf8"));
const players = dataset.teams.flatMap(team => team.players);

assert.equal(dataset.provider, "La Gazzetta dello Sport");
assert.equal(dataset.sourceUrl, "https://www.gazzetta.it/Calcio/prob_form/");
...
assert.ok(dataset.teams.every(team => team.updatedAt), "timestamp Gazzetta mancante su una formazione");
...
console.log(`Gazzetta: ${dataset.teams.length} squadre, ${players.length} titolari, ${dataset.coverage.linkedPlayers} collegati.`);
