...
assert.deepEqual(Object.fromEntries(["very-high", "medium-high", "lower"].map(band => [band, firstMatchday.filter(fixture => fixture.probableFormation.lineupConfidence.band === band).length])), { "very-high": 6, "medium-high": 6, "lower": 6 });
assert.deepEqual(firstMatchday.flatMap(fixture => fixture.probableFormation.lineupConfidence.uncertainSides.map(side => fixture.probableFormation[side].team)), ["AEK Athens", "Inter", "Sporting CP", "Roma", "Leipzig", "Manchester United"]);
...
assert.equal(firstMatchday.find(fixture => fixture.id === "ucl-2026-27-md01-07").probableFormation.home.players[0], "Joan García");
assert.equal(firstMatchday.find(fixture => fixture.id === "ucl-2026-27-md01-11").probableFormation.home.players[9], "Francisco Trincão/Gonçalves");
assert.match(firstMatchday.find(fixture => fixture.id === "ucl-2026-27-md01-12").probableFormation.home.notes[0], /Buongiorno e McTominay.*fuori/);
assert.match(firstMatchday.find(fixture => fixture.id === "ucl-2026-27-md01-13").probableFormation.away.notes[0], /Dybala e Soulé dietro Malen/);
assert.match(firstMatchday.find(fixture => fixture.id === "ucl-2026-27-md01-17").probableFormation.home.notes[0], /Rashford.*dubbio/);
