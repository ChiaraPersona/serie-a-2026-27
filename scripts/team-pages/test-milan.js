...
for (const contract of ["Tiri totali", "Tiri nello specchio", "Falli commessi", "Falli subiti", "goalsPer90", "assistsPer90", "shotsPer90", "shotsOnTargetPer90", "cardsPer90", "foulsCommittedPer90", "foulsWonPer90", "squad-table-wrap", "squadLeaderboards", "Top 3 per statistica", "leader-player", "player-detail", "searchKey", "da verificare"]) assert.ok(interfaceSource.includes(contract), `Interfaccia: contratto ${contract} assente`);
for (const contract of [".squad-table-wrap", "position:sticky", ".squad-table thead tr:nth-child(2) th", ".squad-table tbody tr:hover td", ".squad-table .goal-stat", ".squad-leader-grid", ".squad-leader-card"]) assert.ok(stylesSource.includes(contract), `Stili tabella: contratto ${contract} assente`);
