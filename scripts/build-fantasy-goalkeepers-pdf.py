...
OUTPUT = ROOT / "output" / "pdf" / "fantacalcio-listone-consigli-portieri.pdf"
...
def text_value(value):
    return str(int(value) if finite(value) and float(value).is_integer() else value) if finite(value) else "N/D"
...
def build_rows():
    ...
    rows = []
    for quote in (player for player in quotations["players"] if player["role"] == "P"):
        ...
        external_entries = sorted(external.get("entries", []), key=lambda entry: entry.get("appearances") or 0, reverse=True)
        ...
        if finite(fantasy_appearances) and fantasy_appearances > 0:
            ...
            matching_external = next((entry for entry in external_entries if entry.get("appearances") == appearances), {})
            ...
            goals_conceded = fantasy.get("goalsAgainst")
            ...
            goals_conceded = roster_totals.get("goalsConceded")
            ...
            goals_conceded = external_best.get("goalsConceded")
            ...
            appearances = minutes = goals_conceded = yellow = red = None
        ...
        rows.append({
            ...
            "cards": f"{text_value(yellow)} G / {text_value(red)} R" if finite(yellow) else "N/D",
        })
...
def page_footer(canvas, document):
    ...
    canvas.drawString(18 * mm, 5.8 * mm, "Serie A 2026/27 - Fantacalcio | Dati storici 2025/26")
    canvas.drawRightString(width - 18 * mm, 5.8 * mm, f"Pagina {document.page}")
    canvas.restoreState()


def build_pdf():
    rows = build_rows()
    covered = sum(finite(row["goalsConceded"]) for row in rows)
    ...
    document = SimpleDocTemplate(
        ...
        topMargin=13 * mm,
        bottomMargin=15 * mm,
        title="Fantacalcio - Listone e consigli portieri",
        author="Serie A 2026/27",
        subject="Listone portieri con statistiche 2025/26",
    )
    ...
    heading = [
        ...
        [Paragraph("LISTONE E CONSIGLI", ParagraphStyle("Kicker", parent=body_bold, fontSize=8, textColor=BLUE, leading=10)), Paragraph("Portieri", title_style)],
        Paragraph(f"{len(rows)} profili<br/><font size=8 color='{MUTED.hexval()}'>{covered} con gol subiti disponibili</font>", ParagraphStyle("Summary", parent=body_bold, fontSize=13, leading=15, textColor=NAVY, alignment=TA_RIGHT)),
    ]
    ...
    story.extend([
        header_table,
        Paragraph("Quotazioni e indicatori del listone 2026/27; presenze, minuti, gol subiti e cartellini si riferiscono ai campionati nazionali 2025/26.", subtitle_style),
    ])

    headers = ["#", "Stelle", "Portiere", "Squadra", "Indice", "Qt.", "FVM", "Val. mercato", "Pres.", "Minuti", "Gol subiti", "Cartellini"]
    table_data = [[Paragraph(header, ParagraphStyle(f"Head{index}", parent=body_bold, fontSize=7.3, leading=8, textColor=colors.white, alignment=TA_RIGHT if index >= 4 and index != 7 and index != 11 else TA_LEFT)) for index, header in enumerate(headers)]]
    for index, row in enumerate(rows, 1):
        table_data.append([
            ...
            cell(text_value(row["goalsConceded"]), right_style, row["goalsConceded"] is None),
            cell(row["cards"], body_style, row["cards"] == "N/D"),
        ])

    widths = [8 * mm, 14 * mm, 39 * mm, 24 * mm, 15 * mm, 11 * mm, 12 * mm, 24 * mm, 12 * mm, 16 * mm, 18 * mm, 25 * mm]
    ...
    story.append(Paragraph(
        "Fonti: listone e statistiche Fantacalcio, statistiche squadra ed ESPN Core. Priorità per i gol subiti: Fantacalcio, dato squadra del calciatore, ESPN Core. I valori non verificabili restano N/D; nessuno zero è stato stimato.",
        note_style,
    ))
    story.append(Spacer(1, 2 * mm))
    story.append(Paragraph(f"PDF generato il {datetime.now().strftime('%d/%m/%Y alle %H:%M')}", note_style))
    document.build(story, onFirstPage=page_footer, onLaterPages=page_footer)
    print(f"Creato {OUTPUT} ({len(rows)} portieri, {covered} gol subiti disponibili).")
