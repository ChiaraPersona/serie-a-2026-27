...
assert.deepEqual(data.slips.map(slip=>slip.legs.length),[8,8,10,4,4,6,6,6]);
assert.deepEqual(data.slips.map(slip=>slip.name),["Otto tiri e tiri in porta","Otto gol e assist","Multigol casa/ospite · 10 partite","Poker ammoniti 1","Poker ammoniti 2","Martedì Champions · 6 partite","Mercoledì Champions · 6 partite","Giovedì Champions · 6 partite"]);
...
for(const evening of data.slips.slice(5)){
assert.ok(evening.combinedOdds>5);
assert.deepEqual(evening.legs.map(x=>x.matchId).sort(),calendar.fixtures.filter(x=>x.date===evening.date).map(x=>x.id).sort());
