...
assert(predictions.engine?.version&&predictions.predictions?.length===33,"Il motore pronostici deve coprire le 30 gare delle prime tre giornate e le tre Letture di Coppa");
...
assert(["r16-3","r16-6","r16-7"].every(matchId=>predictions.predictions.some(prediction=>prediction.matchId===matchId&&prediction.market.status==="unavailable"&&prediction.combinations.length===0)),"I tre pronostici Coppa devono usare il modello senza inventare quote o MyCombo");
...
assert(!readings.some(item=>item.matchId==="r16-4")&&["r16-3","r16-6","r16-7"].every(matchId=>{const reading=readings.find(item=>item.matchId===matchId);return reading?.status==="published"&&readingModules.filter(moduleId=>reading.sections[moduleId].content).length===6&&reading.sections.market.content===null}),"La Lettura conclusa Parma-Cremonese deve essere rimossa e le altre tre Letture di Coppa devono restare complete");
assert(["r16-3","r16-6","r16-7"].every(matchId=>{const match=cup.matches.find(item=>item.id===matchId);return match?.refereeAssignment?.referee?.slug&&match.refereeAssignment.assistants?.length===2&&match.sources.some(source=>source.sourceType==="aia-figc-designations")}),"Le tre Letture di Coppa devono avere la designazione arbitrale strutturata e la fonte AIA");
assert((match=>match?.status==="finished"&&match.score?.home===0&&match.score?.away===2&&match.winner==="Cremonese"&&match.scorers?.length===2&&match.resultSource?.url)(cup.matches.find(match=>match.id==="r16-4")),"Risultato Parma-Cremonese non valido");
