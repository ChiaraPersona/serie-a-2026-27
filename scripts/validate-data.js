...
const fantasySourcePlayersWithVote=new Set(fantasyWorkbook.players.filter(player=>player.playerId&&player.appearancesWithVote>0&&player.averageRating>0&&player.fantasyAverage>0).map(player=>player.playerId));
const fantasyPlayersWithPublishedRatings=fantasy.players.filter(player=>fantasySourcePlayersWithVote.has(player.id));
const fantasyPlayersWithIntegratedRatings=fantasy.players.filter(player=>player.fantasyScoring.averageRating!==null&&player.fantasyScoring.fantasyAverage!==null);
assert(fantasyPlayersWithIntegratedRatings.length===fantasyPlayersWithPublishedRatings.length&&fantasyPlayersWithIntegratedRatings.every(player=>fantasySourcePlayersWithVote.has(player.id)),"PV, MV e FM non riconciliati con i profili attivi coperti dalla fonte");
assert(fantasy.sources.fantasyStatistics.matchedPlayersWithVote===fantasyPlayersWithIntegratedRatings.length,"Copertura PV, MV e FM non dichiarata correttamente nella metodologia");
...
for(const matchId of ["monza-sassuolo-2026-27-md-05","roma-inter-2026-27-md-05"]){const match=finishedLeagueMatches.find(item=>item.id===matchId);assert(match?.resultCoverage?.playerStats==="complete"&&match.playerStats.home.length===16&&match.playerStats.away.length===16,`Statistiche giocatore MD5 non complete: ${matchId}`)}
for(const matchId of ["bologna-torino-2026-27-md-05","udinese-cagliari-2026-27-md-05"]){const match=finishedLeagueMatches.find(item=>item.id===matchId);assert(match?.resultCoverage?.playerStats==="unavailable"&&match.playerStats.home.length===0&&match.playerStats.away.length===0,`Copertura giocatore non verificabile non esplicita: ${matchId}`)}
...
const prototypeReadings=readings.filter(reading=>reading.prototype),firstFiveMatchdayIds=new Set(league.filter(match=>match.matchday>=1&&match.matchday<=5).map(match=>match.id));
assert(prototypeReadings.length===50&&prototypeReadings.every(reading=>firstFiveMatchdayIds.has(reading.matchId)),"Le Letture devono coprire le 50 gare delle prime cinque giornate");
...
assert(atalantaSassuoloReading.sections.market.content===null&&atalantaSassuoloReading.sections.market.sources.length===0,"La lettura pilota conclusa non deve essere riscritta con lo snapshot quote di una giornata successiva");
assert(atalantaSassuoloReading.sections.referee.content&&atalantaSassuoloReading.sections.referee.sources.length,"La lettura pilota deve includere la designazione arbitrale AIA");
assert(atalantaSassuoloReading.sections.synthesis.content===null,"La lettura pilota non deve inventare la sintesi mancante");
assert(predictions.engine?.version&&predictions.predictions?.length===39,"Il motore pronostici deve conservare gli snapshot precedenti e coprire la quinta giornata di Serie A");
...
for(const prediction of predictions.predictions){const values=Object.values(prediction.probabilities.final);assert(readingMatchIds.has(prediction.matchId),`Partita pronostico non trovata: ${prediction.matchId}`);assert(Number(values.reduce((total,value)=>total+value,0).toFixed(1))===100,`Probabilita pronostico non esattamente normalizzate: ${prediction.matchId}`);assert(prediction.exactScores.length===3,`Risultati esatti incompleti: ${prediction.matchId}`);assert(prediction.scoreForecast?.primary?.score&&prediction.scoreForecast?.display?.length===3&&prediction.scoreForecast.coherentWithVerdict===(prediction.scoreForecast.primary.outcome===prediction.verdict.outcome),`Risultato principale non valido: ${prediction.matchId}`);assert(prediction.scoreProfile?.bands?.length===3,`Profilo punteggi incompleto: ${prediction.matchId}`);assert(prediction.surprise.value>=0&&prediction.surprise.value<=100,`Fattore sorpresa non valido: ${prediction.matchId}`);assert(prediction.teamProjections?.length===2,`Volumi squadra incompleti: ${prediction.matchId}`);assert(prediction.matchProjection&&["shotsTotal","shotsOnTarget","corners"].every(metric=>{const value=prediction.matchProjection[metric];return value&&value.min<=value.central&&value.central<=value.max&&value.interval==="p20-p80-independent"}),`Volumi totali partita non validi: ${prediction.matchId}`);assert(prediction.teamProjections.every(team=>["shotsTotal","shotsOnTarget","corners"].every(metric=>{const value=team[metric];return value&&value.min<=value.central&&value.central<=value.max&&value.interval==="p20-p80"})),`Intervalli squadra non validi: ${prediction.matchId}`);assert(prediction.likelyBooked?.length===5&&new Set(prediction.likelyBooked.map(item=>item.teamId)).size===2,`Gerarchia ammoniti non valida: ${prediction.matchId}`);assert(prediction.mvpCandidate?.name&&prediction.mvpCandidate?.mvpHistory?.status,`MVP non valido: ${prediction.matchId}`);assert(prediction.market.status==="unavailable"||prediction.marketComparison?.length>=16,`Confronto quote incompleto: ${prediction.matchId}`);assert(prediction.scenarios?.length===3,`Scenari incompleti: ${prediction.matchId}`)}
assert(!predictions.predictions.some(prediction=>cup.matches.some(match=>match.id===prediction.matchId)),"I pronostici delle Letture di Coppa rimosse non devono essere presenti");
assert(cup.competition==="coppa-italia"&&cup.season==="2026-27","Dataset Coppa Italia non valido");
assert(cup.source?.url==="https://www.diretta.it/calcio/italia/coppa-italia/calendario/"&&cup.source?.sourceType==="live-score-calendar","Fonte Diretta.it Coppa Italia mancante");
...
assert(cupOpponents.teams[0].roster.length===30&&cupOpponents.teams[0].roster.every(player=>player.name&&player.role&&player.providerPlayerId===null),"Rosa ufficiale Cremonese incompleta o abbinata senza verifica");
assert(!readings.some(reading=>cup.matches.some(match=>match.id===reading.matchId)),"Le Letture di Coppa devono essere rimosse totalmente");
assert(["r16-6","r16-7"].every(matchId=>{const match=cup.matches.find(item=>item.id===matchId);return match?.refereeAssignment?.referee?.slug&&match.refereeAssignment.assistants?.length===2&&match.sources.some(source=>source.sourceType==="aia-figc-designations")}),"Le due gare di Coppa devono conservare la designazione arbitrale strutturata e la fonte AIA");
assert((match=>match?.status==="finished"&&match.score?.home===0&&match.score?.away===2&&match.winner==="Cremonese"&&match.scorers?.length===2&&match.bookings?.length===2&&match.resultSource?.url)(cup.matches.find(match=>match.id==="r16-4")),"Risultato o ammoniti di Parma-Cremonese non validi");
assert((match=>match?.status==="finished"&&match.score?.home===0&&match.score?.away===1&&match.winner==="Monza"&&match.scorers?.length===1&&match.bookings?.length===2&&match.resultSource?.url)(cup.matches.find(match=>match.id==="r16-3")),"Risultato o ammoniti di Torino-Monza non validi");
assert(cup.matches.find(match=>match.id==="oct-4")?.away==="Cremonese","Qualificazione Cremonese agli ottavi non aggiornata");
assert(cup.matches.find(match=>match.id==="oct-3")?.away==="Monza","Qualificazione Monza agli ottavi non aggiornata");
assert(cup.matches.find(match=>match.id==="oct-1")?.away==="Genoa"&&cup.matches.find(match=>match.id==="oct-8")?.away==="Fiorentina","Qualificate Genoa e Fiorentina agli ottavi non aggiornate");
assert((match=>match?.status==="finished"&&match.score?.home===1&&match.score?.away===0&&match.winner==="Genoa")(cup.matches.find(match=>match.id==="r16-1")),"Risultato Genoa-Südtirol non valido");
assert((match=>match?.status==="finished"&&match.score?.home===3&&match.score?.away===0&&match.winner==="Fiorentina")(cup.matches.find(match=>match.id==="r16-8")),"Risultato Fiorentina-Pisa non valido");
...
assert(cup.matches.filter(match=>match.stage==="round-16").every(match=>match.date&&match.kickoff&&match.dateStatus==="confirmed"),"Programmazione sedicesimi incompleta");
