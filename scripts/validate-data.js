...
const prototypeReadings=readings.filter(reading=>reading.prototype),firstThreeMatchdayIds=new Set(league.filter(match=>match.matchday>=1&&match.matchday<=3).map(match=>match.id));
assert(prototypeReadings.length===30&&prototypeReadings.every(reading=>firstThreeMatchdayIds.has(reading.matchId)),"Le Letture devono coprire le 30 gare delle prime tre giornate");
...
assert(predictions.engine?.version&&predictions.predictions?.length===32,"Il motore pronostici deve coprire le 30 gare delle prime tre giornate e le due Letture di Coppa ancora da disputare");
...
assert(!readings.some(item=>["r16-3","r16-4"].includes(item.matchId))&&["r16-6","r16-7"].every(matchId=>{const reading=readings.find(item=>item.matchId===matchId);return reading?.status==="published"&&readingModules.filter(moduleId=>reading.sections[moduleId].content).length===6&&reading.sections.market.content===null}),"Le Letture concluse devono essere rimosse e le due gare di Coppa ancora da disputare devono restare complete");
assert(["r16-6","r16-7"].every(matchId=>{const match=cup.matches.find(item=>item.id===matchId);return match?.refereeAssignment?.referee?.slug&&match.refereeAssignment.assistants?.length===2&&match.sources.some(source=>source.sourceType==="aia-figc-designations")}),"Le due Letture di Coppa devono avere la designazione arbitrale strutturata e la fonte AIA");
