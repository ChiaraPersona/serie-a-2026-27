...
for(const prediction of predictions.predictions){const values=Object.values(prediction.probabilities.final);assert(matchIds.has(prediction.matchId),`Partita pronostico non trovata: ${prediction.matchId}`);assert(Math.abs(values.reduce((total,value)=>total+value,0)-100)<=0.2,`Probabilita pronostico non normalizzate: ${prediction.matchId}`);assert(prediction.exactScores.length===3,`Risultati esatti incompleti: ${prediction.matchId}`);assert(prediction.surprise.value>=0&&prediction.surprise.value<=100,`Fattore sorpresa non valido: ${prediction.matchId}`);assert(prediction.teamProjections?.length===2,`Volumi squadra incompleti: ${prediction.matchId}`);assert(prediction.likelyBooked?.length===5&&new Set(prediction.likelyBooked.map(item=>item.teamId)).size===2,`Gerarchia ammoniti non valida: ${prediction.matchId}`);assert(prediction.mvpCandidate?.name,`MVP non valido: ${prediction.matchId}`)}
