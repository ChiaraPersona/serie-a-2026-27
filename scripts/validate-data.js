...
const finishedMd05=finishedLeagueMatches.filter(match=>match.matchday===5);
assert(finishedMd05.length===10,"La quinta giornata deve avere 10 referti conclusi");
assert(finishedMd05.every(match=>match.resultCoverage?.teamStats==="complete"&&match.resultCoverage?.playerStats==="complete"&&match.resultCoverage?.participation==="available"&&match.playerStats.home.length>=11&&match.playerStats.away.length>=11),"Copertura completa MD5 non propagata");
...
assert(predictions.engine?.version&&predictions.predictions?.length===49,"Il motore pronostici deve conservare gli snapshot precedenti e coprire la quinta giornata di Serie A");
