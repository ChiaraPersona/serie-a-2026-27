(() => {
  ...
  const release = "20260824-team-stats-by-season-v10";
  ...
  const tacticalProfileSection = (team, dataset) => {
    ...
    const qualityLabels = { complete: "Profilo completo", "statistical-only": "Solo riepilogo statistico", "limited-sample": "Campione limitato" };
  ...
  const nav = `<header class="subpage-header"><a href="${base}statistiche-squadre.html">← Tutte le squadre</a></header>`;
  ...
  function teamTotalStatsBlock(stats, promoted) {
    ...
    return `<section class="detail-section team-total-section" data-team-season="total"><header class="team-season-heading"><div><p class="eyebrow">Riepilogo complessivo</p><h2>Totale 2025/26 + 2026/27</h2></div><span>${esc(stats.competition)}</span></header>${mixedCompetitionNote}<div class="team-season-group"><h3>Risultati complessivi</h3>${statGrid(stats.results, ["played", "won", "drawn", "lost", "points", "pointsPerGame", "goalsFor", "goalsAgainst", "goalDifference", "cleanSheets", "failedToScore", "winPercentage", "drawPercentage", "lossPercentage"])}</div><div class="team-season-group"><h3>Disciplina complessiva</h3>${statGrid(stats.discipline, ["foulsCommitted", "foulsCommittedPerGame", "foulsWon", "foulsWonPerGame", "yellowCards", "yellowCardsPerGame", "secondYellowCards", "straightRedCards", "dismissals", "penaltiesConceded", "penaltiesWon", "disciplineIndex"])}</div></section>`;
  }
