(() => {
  ...
  const labels = {
    ...
    shots: "Tiri", shotsOnTarget: "Tiri in porta", penaltiesTaken: "Rigori calciati", penaltiesScored: "Rigori segnati",
  ...
  const nav = `<header class="subpage-header"><a href="${base}statistiche-squadra/index.html">← Tutte le squadre</a><a href="${base}statistiche-squadre.html">Riepilogo 2025/26</a></header>`;
  ...
  function indexPage(data) {
    const milan = data.teams.find(team => team.id === "milan");
    const featuredMilan = milan ? `<section class="team-directory-feature"><article class="milan-pilot-card"><img src="${milan.logo}" alt="Stemma del Milan"><div><span class="competition-chip">Pagina completa</span><h2>AC Milan 2026/27</h2><p>${milan.playerCount} calciatori con statistiche 2025/26, valori per 90 minuti e fonti.</p><a class="button button-primary" href="milan.html">Apri la pagina del Milan</a></div></article></section>` : "";
    root.innerHTML = `<section class="team-directory-hero"><p class="eyebrow">Serie A 2026/27</p><h1>Rose e statistiche squadra</h1><p class="lead">20 pagine alimentate da JSON. Campionato 2025/26 sempre indicato, con Serie B separata per le neopromosse.</p></section>${featuredMilan}<section class="team-directory-grid">${data.teams.map(team => `<a class="team-directory-card" href="${team.id}.html"><img src="${team.logo}" alt="Stemma ${esc(team.name)}"><div><span class="competition-chip">${esc(team.previousSeason.competition)} 2025/26</span><h2>${esc(team.name)}</h2><p>${team.previousSeason.position ? `${team.previousSeason.position}ª posizione · ${team.previousSeason.points} punti` : "Posizione N/D"}</p><small>Allenatore: ${value(team.coach)} · Rosa verificata: ${team.playerCount}</small></div><strong aria-hidden="true">→</strong></a>`).join("")}</section>`;
  }

  const filters = () => `<div class="squad-controls"><label>Ricerca<input id="player-search" type="search" placeholder="Nome calciatore"></label><label>Ruolo<select id="role-filter"><option value="">Tutti</option><option>Portiere</option><option>Difensore</option><option>Centrocampista</option><option>Attaccante</option></select></label><label>Stato<select id="status-filter"><option value="">Tutti</option><option>confermato</option><option>nuovo acquisto</option><option>prestito</option><option>rientro dal prestito</option><option>primavera</option><option>da verificare</option></select></label><label>Ordina<select id="player-sort"><option value="appearances">Presenze</option><option value="minutes">Minuti</option><option value="goals">Gol</option><option value="assists">Assist</option><option value="shots">Tiri</option><option value="shotsOnTarget">Tiri in porta</option><option value="foulsCommitted">Falli commessi</option><option value="cards">Cartellini</option><option value="age">Età</option></select></label></div>`;
  ...
  function squadTable(players) {
    ...
    return `<p class="squad-count" aria-live="polite">${players.length} calciator${players.length === 1 ? "e" : "i"}</p><div class="table-wrap"><table class="squad-table"><thead><tr><th>Calciatore</th><th>Ruolo</th><th>Stato</th><th>Squadra 2025/26</th><th>Competizione</th><th>PG</th><th>Min</th><th>Gol</th><th>Assist</th><th>Tiri</th><th>Falli</th><th>Cart.</th></tr></thead><tbody>${players.map(player => {
      const entry = player.previousSeason?.entries?.find(item => item.competitionType === "domestic-league") || player.previousSeason?.entries?.[0] || {};
      return `<tr data-player-id="${esc(player.id)}"><td><button class="player-open" type="button" data-player-id="${esc(player.id)}"><span class="mini-avatar">${initials(player.name)}</span><span><strong>${esc(player.name)}</strong><small>${value(player.shirtNumber)} · ${value(player.nationality)}</small></span></button></td><td>${value(player.detailedRole || player.role)}</td><td><span class="status-badge">${value(player.status)}</span></td><td>${value(entry.team)}</td><td>${value(entry.competition)}</td><td>${value(entry.appearances)}</td><td>${value(entry.minutes)}</td><td>${value(entry.goals)}</td><td>${value(entry.assists)}</td><td>${value(entry.shots)}</td><td>${value(entry.foulsCommitted)}</td><td>${value(cards(entry))}</td></tr>`;
