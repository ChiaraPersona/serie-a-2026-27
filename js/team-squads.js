(() => {
  ...
  function indexPage(data) {
    const completed = data.teams.filter(team => team.playerCount > 0);
    const featuredTeams = completed.length === data.teams.length
      ? `<section class="team-directory-feature"><article class="coverage-complete-card"><div><p class="eyebrow">Copertura completa</p><h2>${completed.length} squadre su ${data.teams.length}</h2><p>Tutte le rose 2026/27 dispongono di tabella individuale, statistiche 2025/26, valori per 90 minuti e Top 3.</p></div><strong>${completed.length}/20</strong></article></section>`
      : `<section class="team-directory-feature"><div class="completed-team-heading"><p class="eyebrow">Pagine complete</p><h2>Rose, statistiche per 90’ e Top 3</h2></div><div class="completed-team-grid">${completed.map(team => `<article class="completed-team-card"><img src="${team.logo}" alt="Stemma ${esc(team.name)}"><div><span class="competition-chip">${team.playerCount} calciatori</span><h3>${esc(team.name)} 2026/27</h3><p>Statistiche 2025/26, valori per 90 minuti e fonti registrate.</p><a class="button button-primary" href="${team.id}.html">Apri ${esc(team.name)}</a></div></article>`).join("")}</div></section>`;
    root.innerHTML = `<section class="team-directory-hero"><p class="eyebrow">Serie A 2026/27</p><h1>Rose e statistiche squadra</h1><p class="lead">20 pagine alimentate da JSON. Campionato 2025/26 sempre indicato, con Serie B separata per le neopromosse.</p></section>${featuredTeams}<section class="team-directory-grid">${data.teams.map(team => `<a class="team-directory-card" href="${team.id}.html"><img src="${team.logo}" alt="Stemma ${esc(team.name)}"><div><span class="competition-chip">${esc(team.previousSeason.competition)} 2025/26</span><h2>${esc(team.name)}</h2><p>${team.previousSeason.position ? `${team.previousSeason.position}ª posizione · ${team.previousSeason.points} punti` : "Posizione N/D"}</p><small>Allenatore: ${value(team.coach)} · Rosa disponibile: ${team.playerCount}</small></div><strong aria-hidden="true">→</strong></a>`).join("")}</section>`;
  }
