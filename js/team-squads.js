(() => {
  ...
  function teamSeasonStatsColumn(stats) {
    ...
    return `<details class="team-season-column" data-team-season="${esc(stats.season)}"${current ? " open" : ""}><summary class="team-season-heading"><div><span class="eyebrow">${esc(stats.competition)}</span><h2>Statistiche ${esc(stats.season)}</h2></div><span>${value(stats.results.played)} partite concluse</span></summary><div class="team-season-content">${noMatches}<details class="team-season-group" open><summary>Risultati</summary>${teamSeasonList(stats.results, ["played", "won", "drawn", "lost", "points", "pointsPerGame", "goalsFor", "goalsAgainst", "goalDifference", "cleanSheets", "failedToScore", "winPercentage", "drawPercentage", "lossPercentage"])}</details><details class="team-season-group"><summary>Disciplina</summary>${teamSeasonList(stats.discipline, ["foulsCommitted", "foulsCommittedPerGame", "foulsWon", "foulsWonPerGame", "yellowCards", "yellowCardsPerGame", "secondYellowCards", "straightRedCards", "dismissals", "penaltiesConceded", "penaltiesWon", "disciplineIndex"])}</details><details class="team-season-group"><summary>Casa e trasferta</summary><div class="team-season-splits"><section><h4>Casa</h4>${teamSeasonList(stats.homeAway.home, ["played", "won", "drawn", "lost", "goalsFor", "goalsAgainst", "yellowCards"])}</section><section><h4>Trasferta</h4>${teamSeasonList(stats.homeAway.away, ["played", "won", "drawn", "lost", "goalsFor", "goalsAgainst", "yellowCards"])}</section></div></details></div></details>`;
  }
  ...
  function teamPage(team, objectiveDataset, standings, teams, matches, teamStyleProfiles, europeanFixtures) {
    ...
    const calendarSection = personalCalendar(team, teams, matches, europeanFixtures);
