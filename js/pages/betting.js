...
export function createPage(deps){
  ...
  function myComboRoundContent(predictions,matchById,teamById){
    ...
    const cards=entries.map(({combo,match})=>{
      ...
      const metrics=finished?`<div class="betting-slip-total"><span>Esiti presi</span><strong>${won}</strong><small>${decided} esiti verificati</small></div><div><span>Da verificare</span><strong>${pending}/${results.length}</strong></div>`:`<div class="betting-slip-total"><span>Quota selezionata</span><strong data-mycombo-total>1,00</strong><small data-mycombo-count>0 esiti selezionati</small></div><div><span>Scelta</span><strong data-mycombo-selected>0/${combo.legs.length}</strong></div>`;
      const legs=combo.legs.map((leg,index)=>{const result=results[index],settled=finished&&["won","lost","void"].includes(result.status),status=finished?(settled?result.status:"unavailable"):"pending",badge=finished?`<span class="betting-leg-result">${esc(settled?result.label:"Da verificare")}</span>`:"";return `<li${finished?` class="betting-leg--${status}" data-settlement="${status}"`:""}><button type="button" class="betting-mycombo-pick" data-mycombo-pick data-odds="${Number(leg.odds)}" aria-pressed="false"${finished?" disabled":""}><span class="betting-leg-number">${String(index+1).padStart(2,"0")}</span><span class="betting-mycombo-pick-copy"><strong>${esc(leg.market)}</strong><span>${esc(leg.label)}</span>${badge}</span><b>${odds(leg.odds)}</b></button></li>`}).join("");
      return `<article class="betting-slip betting-mycombo-card" data-match-id="${esc(match.id)}"${finished?' data-finished="true"':""}><header><div><p>MyCombo · ${finished?"risultati":`scegli tra ${combo.legs.length} esiti`}</p><h2>${esc(home)} - ${esc(away)}</h2><small>${finished?`Finale ${match.score.home}-${match.score.away} · verde = esito preso`:"Seleziona o deseleziona gli esiti da combinare · quota minima 1,15"}</small></div></header><div class="betting-slip-metrics">${metrics}<div><span>Rischio</span><strong>${esc(combo.risk||combo.tier)}</strong></div></div><ol class="betting-mycombo-options">${legs}</ol><p class="betting-weakest">${finished?"Esiti verificati sul referto finale; i mercati non coperti dai dati disponibili restano da verificare.":"Questi pulsanti appartengono esclusivamente al costruttore MyCombo; le schedine sottostanti restano separate."}</p></article>`;
    }).join("");
    return `<section class="betting-mycombo-round" aria-labelledby="betting-mycombo-md05-title"><header class="betting-section-heading"><div><p class="eyebrow">Una per ogni partita</p><h3 id="betting-mycombo-md05-title">MyCombo · fino a 10 esiti per gara</h3></div><p>Le proposte restano quelle dello snapshot pre-partita. Per le gare concluse gli esiti verificabili sono aggiornati sul referto: quelli presi sono evidenziati in verde, mentre i dati non coperti restano da verificare.</p></header><div class="betting-slip-grid">${cards}</div></section>`;
  }

  function bindMyComboInteractions(){
    document.querySelectorAll('.betting-mycombo-card:not([data-finished="true"])').forEach(card=>{
      const picks=[...card.querySelectorAll("[data-mycombo-pick]")];
      const total=card.querySelector("[data-mycombo-total]");
      const count=card.querySelector("[data-mycombo-count]");
      const selected=card.querySelector("[data-mycombo-selected]");
      const update=()=>{
        const active=picks.filter(pick=>pick.getAttribute("aria-pressed")==="true");
        const combined=active.reduce((value,pick)=>value*Number(pick.dataset.odds),1);
        total.textContent=odds(combined);
        count.textContent=`${active.length} ${active.length===1?"esito selezionato":"esiti selezionati"}`;
        selected.textContent=`${active.length}/${picks.length}`;
      };
      picks.forEach(pick=>pick.addEventListener("click",()=>{pick.setAttribute("aria-pressed",pick.getAttribute("aria-pressed")!=="true"?"true":"false");update();}));
      update();
    });
  }
  ...
  async function render(){
    ...
    document.querySelector("#app").innerHTML=hero("Archivio · Stagione 2026/27","Schedina","Le schedine sono raccolte separatamente giornata per giornata; le MyCombo della quinta giornata si trovano nella relativa pagina dedicata.")+`<section class="betting-archive" aria-labelledby="betting-archive-title"><header class="betting-archive-intro"><div><p class="eyebrow">Archivio schedine</p><h2 id="betting-archive-title">Competizioni e giornate</h2></div><p>La prima card blu raccoglie la Champions; seguono le giornate di Serie A.</p></header><div class="betting-archive-list team-directory-grid team-flip-grid">${championsArchiveCard(champions)}${archiveCard(md1,1,matchById)}${archiveCard(md2,2,matchById)}${archiveCard(md3,3,matchById)}${archiveCard(md4,4,matchById)}${archiveCard(md5,5,matchById)}</div></section>`;
  }
  return {render};
}
