...
export function createPage(deps){
  ...
  async function render(){
    ...
    const archiveSettlements=data.slips.flatMap(slip=>slip.legs.map(leg=>settleLeg(leg,matchById.get(leg.matchId))));
    const archiveWins=archiveSettlements.filter(result=>result.status==="won").length;
    const archiveSuccessPct=archiveSettlements.length?archiveWins/archiveSettlements.length*100:0;
