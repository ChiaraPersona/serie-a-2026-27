...
export function createPage(deps){
  ...
  function myComboRoundContent(predictions,matchById,teamById){
    ...
    const cards=entries.map(({combo,match})=>{
      ...
      const results=combo.legs.map(leg=>settleLeg(leg,match)),won=results.filter(result=>result.status==="won").length,decided=results.filter(result=>["won","lost","void"].includes(result.status)).length,pending=results.length-decided;
