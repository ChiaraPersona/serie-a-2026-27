...
export function createPage(deps){
  ...
  function championsArchiveCard(data){
    return `<a class="betting-archive-card betting-archive-card--champions team-directory-card team-flip-card" href="schedina.html?competizione=champions" aria-label="Apri la schedina Champions League" style="--team-primary:#0756c9;--team-secondary:#02183f"><span class="team-flip-inner"><span class="team-flip-face team-flip-front betting-archive-card-front betting-champions-front" style="background:radial-gradient(circle at 70% 18%,rgba(80,196,255,.5),transparent 42%),linear-gradient(145deg,#0966e8,#031b4b)!important;color:#fff"><span class="betting-archive-number">CL1</span><small>UEFA</small></span><span class="team-flip-face team-flip-back betting-archive-card-back"><strong>Champions League</strong><span>1ª giornata · 2026/27</span><span>${data.slips.length} schedine</span><span class="betting-archive-performance"><span><small>Selezioni</small><strong>${data.summary.legs}</strong></span><span><small>Partite</small><strong>${data.summary.distinctFixtures}</strong></span></span><span>Quote aggiornate al ${esc(dateOnly(data.oddsRetrievedAt))}</span><b>Apri le schedine Champions</b></span></span></a>`;
  }
  ...
  function championsSlipCard(slip){
    ...
    return `<article class="betting-slip betting-slip--champions betting-slip--${esc(slip.id)}" data-quality="${esc(slip.qualityStatus)}"><header><div><p>${esc(slip.eyebrow)}</p><h2>${esc(slip.name)}</h2><small>${families}</small></div><span class="betting-quality">${esc(slip.qualityLabel)}</span></header><div class="betting-slip-metrics"><div class="betting-slip-total"><span>Quota totale</span><strong>${odds(slip.combinedOdds)}</strong><small>${slip.legs.length} giocate · solo riferimento</small></div><div><span>Probabilità</span><strong>${metric(slip.jointModelProbabilityPct,pct,"%")}</strong></div><div><span>Quota equa</span><strong>${metric(slip.fairOdds,odds)}</strong></div><div><span>EV stimato</span><strong>${metric(slip.expectedValuePct,pct,"%")}</strong></div></div><ol>${slip.legs.map((leg,index)=>`<li><span class="betting-leg-number">${String(index+1).padStart(2,"0")}</span><div><strong>${esc(leg.fixture)}</strong><span>${esc(leg.label)} <small>· ${esc(leg.evidenceLabel)}</small></span></div><b>${odds(leg.odds)}</b></li>`).join("")}</ol>${weak}</article>`;
  }

  function championsContent(data){
    const qualified=data.slips.filter(slip=>slip.qualityStatus==="qualificata"),others=data.slips.filter(slip=>slip.qualityStatus!=="qualificata");
    return `<div class="betting-stage betting-champions-stage"><header class="betting-intro"><div><p class="eyebrow">Champions League · 1ª giornata</p><h3>Otto schedine costruite dal modello</h3></div><p>La stessa matrice della Serie A, con due poker ammoniti in aggiunta. Sono escluse soltanto le schedine a risultato esatto fisso e multiesito.</p></header><p class="betting-coverage"><strong>${data.summary.qualifiedProfiles} proposte qualificate su ${data.slips.length} profili.</strong> ${data.summary.legs} selezioni complessive su ${data.summary.distinctFixtures} gare; le quote vengono associate dopo la scelta del modello.</p>${qualified.length?`<div class="betting-slip-grid betting-slip-grid-qualified">${qualified.map(championsSlipCard).join("")}</div>`:""}${others.length?`<header class="betting-section-heading"><div><p class="eyebrow">Letture editoriali e laboratorio</p><h3>Scenari non qualificati</h3></div><p>Restano visibili per confronto, con probabilità, quota equa ed EV dichiarati.</p></header><div class="betting-slip-grid">${others.map(championsSlipCard).join("")}</div>`:""}<footer class="betting-method"><strong>Criterio Champions</strong><p>${esc(data.selectionRule)}</p><p>${esc(data.methodology)}</p><p>Quote ${esc(data.provider)} aggiornate al ${esc(dateOnly(data.oddsRetrievedAt))}. Esclusi soltanto risultati esatti fissi e risultati esatti multiesito. Gioca responsabilmente.</p></footer></div>`;
  }
  ...
  async function render(){
    ...
    if(competition==="champions"){
      document.querySelector("#app").innerHTML=hero("UEFA Champions League · 2026/27","Schedine Champions","Sei schedine con le regole della Serie A e due poker di probabili ammoniti; le quote Sisal restano un riferimento esterno.")+`<nav class="betting-round-back" aria-label="Navigazione archivio schedine"><a href="schedina.html">← Tutte le schedine</a></nav><section class="betting-round-page betting-round-page--champions"><header class="betting-round-heading"><p class="eyebrow">Champions League · 2026/27</p><h2>1ª giornata</h2></header>${championsContent(champions)}</section>`;
      return;
    }
    if(rounds[matchday]){
