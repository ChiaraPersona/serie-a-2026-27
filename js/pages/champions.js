export function createPage(deps){
  ...
  function pilotReadingDetail(fixture,pilot,backtest,squads,branding,h2h,motivationEntry,styleProfiles,attackByTeam,surpriseEntry,playerMarketEntry,playerMarketDataset){
    ...
    const comboCard=combo=>combo.qualityStatus==="nd"||!combo.legs?.length?`<article class="prediction-combo-card prediction-combo-card-nd"><span>${esc(combo.scenario)} · rischio ${esc(combo.risk)}</span><div><strong>N/D</strong><small>${esc(combo.unavailableReason)}</small></div></article>`:`<article class="prediction-combo-card" data-quality="${esc(combo.qualityStatus)}"><span>${esc(combo.scenario)} · rischio ${esc(combo.risk)}</span><strong>Quota Sisal ${combo.odds.toFixed(2)} · riferimento ≈ ${combo.targetOdds.toFixed(2)}</strong><ul class="prediction-combo-legs">${combo.legs.map(leg=>`<li><span>${esc(leg.label)}${leg.replacementIncluded?" · sostituto incluso":""}</span><b>@ ${leg.odds.toFixed(2)}</b></li>`).join("")}</ul>${Number.isFinite(combo.prudentProbabilityPct)?`<dl class="prediction-combo-metrics"><div><dt>Prob. prudenziale</dt><dd>${combo.prudentProbabilityPct}%</dd></div><div><dt>Quota equa</dt><dd>${combo.fairOdds.toFixed(2)}</dd></div></dl>`:""}<p>${esc(combo.probabilityMethod)}</p></article>`;
    const myCombo=playerMarketEntry?`<section class="section reading-panel-grid champions-reading-combo"><section class="reading-data-panel reading-data-panel-wide"><span>MyCombo</span><p class="champions-combo-policy">Selezioni determinate dal modello; quote Sisal collegate in un secondo momento e usate soltanto come numero.</p><div class="prediction-combos">${playerMarketEntry.combinations.map(comboCard).join("")}</div><small>Quote aggiornate ${esc(shortDate(String(playerMarketDataset?.oddsRetrievedAt||"").slice(0,10)))}. Esclusi DNB, confronti tiri, prima a corner, quasi ammonito e mercati non compatibili.</small></section></section>`:"";
