export function createPage(deps){
  ...
  function pilotReadingDetail(fixture,pilot,backtest,squads,branding,h2h,motivationEntry,styleProfiles){
    ...
    const goalForecast=`<article class="prediction-match-volume champions-goal-forecast"><div><p class="eyebrow">Pronostico quantitativo</p><h3>Distribuzione dei gol</h3></div><dl><div><dt>Gol attesi medi</dt><dd>${fixture.expectedGoals.total.toFixed(2)} <small>${fixture.expectedGoals.home.toFixed(2)}–${fixture.expectedGoals.away.toFixed(2)}</small></dd></div><div><dt>Fascia gol probabile</dt><dd>${fixture.goalBand.min}–${fixture.goalBand.max} <small>${fixture.goalBand.probabilityPct.toFixed(1)}%</small></dd></div></dl><p class="champions-goal-explainer">I gol attesi sono la media di tutti gli scenari, non un secondo risultato esatto.</p></article>`;
    ...
    return `<nav class="reading-back champions-reading-back"><a href="champions-league.html">← Tutte le letture</a><span>Giornata 1</span></nav>
      <section class="reading-match-hero champions-reading-match-hero" aria-labelledby="champions-reading-title"><div><p class="eyebrow">UEFA Champions League · Giornata 1 · prepartita</p><h1 id="champions-reading-title">${esc(fixture.homeTeam)} - ${esc(fixture.awayTeam)}</h1><p>${esc(dayLabel(fixture.date))} · ${esc(fixture.kickoff)}</p></div><div class="reading-matchup champions-reading-matchup" aria-label="${esc(fixture.homeTeam)} contro ${esc(fixture.awayTeam)}">${teamBadge(fixture.homeTeam,branding,{showName:false})}<b>VS</b>${teamBadge(fixture.awayTeam,branding,{showName:false})}</div><div class="reading-summary reading-summary-prototype reading-hero-summary reading-result-summary champions-reading-summary"><div class="reading-exact-scores"><span>3 risultati esatti possibili</span><div class="reading-score-list">${scoreCards}</div></div><div class="reading-hero-signal reading-hero-surprise"><span>Fattore sorpresa</span><strong>N/D</strong><small>Dato non disponibile</small></div><div class="reading-hero-signal"><span>Confidenza</span><strong>${confidence}</strong><small>${esc(fixture.dataQuality.label)}</small></div></div></section>
      <section class="section reading-projection-prototype prediction-volume-section champions-reading-volume" id="champions-forecast"><header class="section-heading"><div><p class="eyebrow">Pronostico e dati di gara</p><h2>Scenario principale</h2></div><p>Gol attesi e fascia più probabile prima dei dati secondari.</p></header>${goalForecast}<section class="champions-reading-volume-block" id="champions-volumes"><header class="section-heading"><div><p class="eyebrow">Proiezione per squadra</p><h2>Gol, tiri, specchio, corner, falli e cartellini</h2></div><p>Storico distinto tra casa e trasferta.</p></header>${matchProjection}<div class="prediction-volume-grid">${fixture.teamProjections.map(teamProjection).join("")}</div><p class="objective-method">Tiri e corner combinano produzione per sede, valori concessi dall'avversaria e ultime otto gare; le fasce sono percentili p20–p80, non margini fissi. La designazione arbitrale è verificata ma non viene applicata come moltiplicatore automatico dei cartellini. I dati mancanti restano N/D.</p></section></section>
  ...
  function championsTeamDetail(team,strength,history,pilot,squads,attackProfile){
  ...
  async function render(){
    ...
    if(requestedTeam){
      document.querySelector("#app").innerHTML=championsTeamDetail(requestedTeam,profiles.get(requestedTeam.team),histories.get(requestedTeam.team),pilot,squads,attackByTeam.get(requestedTeam.team));
      return;
    }
