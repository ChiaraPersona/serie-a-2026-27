export function createPage(deps){
  ...
  const attackChannelsPanel=profile=>{
    ...
    if(!channels)return `<div class="prediction-channels prediction-channels-unavailable"><span>Lati d'attacco</span><p><strong>N/D</strong> · profilo tattico WhoScored non integrato per questa squadra.</p></div>`;
  ...
  function tacticalBaselinePanel(fixture,styleProfiles,branding){
    ...
    const card=team=>{
      ...
      if(!profile)return `<article class="reading-tactical-card champions-reading-tactical-card">${teamBadge(team,branding)}<span class="sample-label sample-low">Profilo non disponibile</span><div class="reading-panel-empty"><strong>N/D</strong><p>Baseline tattica WhoScored non integrata per questa squadra.</p></div>${attackChannelsPanel(null)}</article>`;
      ...
      return `<article class="reading-tactical-card champions-reading-tactical-card">${teamBadge(team,branding)}<span class="sample-label sample-${profile.dataQuality==="complete"?"strong":"low"}">${quality}</span><dl><div><dt>Campione</dt><dd>${esc(profile.competition)} · ${profile.summary.appearances} gare</dd></div><div><dt>Modulo più usato</dt><dd>${esc(profile.formation.code||"N/D")}${profile.formation.appearances?` · ${profile.formation.appearances} gare`:""}</dd></div><div><dt>Possesso</dt><dd>${profile.summary.possessionPct===null?"N/D":`${profile.summary.possessionPct}%`}</dd></div><div><dt>Tiri / gara</dt><dd>${profile.summary.shotsPerGame??"N/D"}</dd></div><div><dt>Gol / gara</dt><dd>${profile.derived.goalsPerGame}</dd></div></dl><div class="reading-tactical-traits">${list("Punti di forza",profile.strengths)}${list("Vulnerabilità",profile.weaknesses)}${list("Stile",profile.playingStyle)}</div>${attackChannelsPanel(profile)}${profile.notes?.length?`<p class="reading-tactical-note">${profile.notes.map(esc).join(" ")}</p>`:""}<a href="${esc(profile.source.url)}" target="_blank" rel="noreferrer">Fonte WhoScored</a></article>`;
    };
    return `<section class="section reading-tactical-baseline champions-reading-tactics" id="champions-tactics"><header class="section-heading"><div><p class="eyebrow">WhoScored · baseline 2025/26</p><h2>Baseline tattica delle squadre</h2></div><p>Stessa struttura delle Letture Serie A; i profili non coperti restano N/D.</p></header><div>${card(fixture.homeTeam)}${card(fixture.awayTeam)}</div><p class="objective-method">Il profilo è contesto storico, non previsione autonoma. Le direzioni d'attacco derivano soltanto dai tratti WhoScored disponibili.</p></section>`;
  }
  ...
  function pilotReadingDetail(fixture,pilot,backtest,squads,branding,h2h,motivationEntry,styleProfiles){
    ...
    const goalForecast=`<article class="prediction-match-volume champions-goal-forecast"><div><p class="eyebrow">Pronostico quantitativo</p><h3>${esc(selection[0])} · ${esc(selection[2])}</h3><small>Copertura ${Number(selection[1]).toFixed(1)}% · probabilità 1/X/2: ${fixture.probabilities.home.toFixed(1)}% · ${fixture.probabilities.draw.toFixed(1)}% · ${fixture.probabilities.away.toFixed(1)}%</small></div><dl><div><dt>Risultato coerente con la selezione</dt><dd>${esc(fixture.scoreForecast.primary.score)}</dd></div><div><dt>Gol attesi medi</dt><dd>${fixture.expectedGoals.total.toFixed(2)} <small>${fixture.expectedGoals.home.toFixed(2)}–${fixture.expectedGoals.away.toFixed(2)}</small></dd></div><div><dt>Fascia gol probabile</dt><dd>${fixture.goalBand.min}–${fixture.goalBand.max} <small>${fixture.goalBand.probabilityPct.toFixed(1)}%</small></dd></div></dl><p class="champions-goal-explainer">I gol attesi sono la media di tutti gli scenari, non un secondo risultato esatto. ${fixture.scoreForecast.modal.score!==fixture.scoreForecast.primary.score?`La moda assoluta è ${esc(fixture.scoreForecast.modal.score)}, ma non rientra nella selezione ${esc(selection[0])}.`:"Il risultato coerente coincide anche con la moda assoluta."}</p><div class="prediction-probability-list">${probabilityRows(fixture.goals)}</div></article>`;
    const styleByTeam=new Map((styleProfiles?.profiles||[]).map(profile=>[profile.teamId,profile]));
    const teamProjection=team=>`<article class="prediction-team-volume champions-reading-team-volume">${teamBadge(team.team,branding)}<div class="prediction-volume-metrics"><div><span>Gol attesi</span><strong>${team.expectedGoals.toFixed(2)}</strong></div><div><span>Tiri totali</span><strong>${esc(metricValue(team.shotsTotal))}</strong></div><div><span>Tiri nello specchio</span><strong>${esc(metricValue(team.shotsOnTarget))}</strong></div><div><span>Corner</span><strong>${esc(metricValue(team.corners))}</strong></div><div><span>Falli</span><strong>${esc(metricValue(team.fouls))}</strong></div><div><span>Cartellini</span><strong>${esc(metricValue(team.cards))}</strong></div></div>${attackChannelsPanel(styleByTeam.get(team.teamId))}</article>`;
    ...
    const bookedPanel=`<section class="prediction-booked-panel"><header><div><p class="eyebrow">Gerarchia disciplinare</p><h2>5 probabili ammoniti</h2></div><p>Graduatoria unica sulle due squadre.</p></header><div class="reading-panel-empty"><strong>N/D</strong><p>Gli undici editoriali sono disponibili, ma mancano dati individuali Champions comparabili per una graduatoria attendibile.</p></div></section>`;
    ...
    return `<nav class="reading-back champions-reading-back"><a href="champions-league.html">← Tutte le letture</a><span>Giornata 1</span></nav>
      ...
      ${tacticalBaselinePanel(fixture,styleProfiles,branding)}
      <section class="champions-reading-context" id="champions-context" aria-labelledby="champions-context-title"><header class="champions-reading-section-heading"><div><p class="eyebrow">Contesto della gara</p><h2 id="champions-context-title">Motivazione e direzione arbitrale</h2></div><p>Indicatori di contesto separati dal pronostico quantitativo.</p></header>${motivationPanel(motivationEntry)}${refereeReadingPanel(fixture.refereeAssignment)}</section>
      <div id="champions-history">${h2hPanel}</div>
      <details class="champions-reading-pending"><summary><span><strong>Analisi in attesa di dati comparabili</strong><small>Ammoniti, candidato MVP e MyCombo restano N/D</small></span><b>Apri</b></summary><div class="prediction-players-grid">${bookedPanel}${mvpPanel}</div>${myCombo}</details>`;
  }
  ...
  async function render(){
    ...
    if(requestedFixture){
      document.querySelector("#app").innerHTML=pilotReadingDetail(requestedFixture,pilot,backtest,squads,branding,h2h,motivationByFixture.get(requestedMatchId),styleProfiles);
      return;
    }
