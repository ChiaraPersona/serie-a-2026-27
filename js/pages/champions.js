export function createPage(deps){
  ...
  function fixtureCard(fixture,branding,pilotByFixture,motivationByFixture){
    const analysis=pilotByFixture.get(fixture.id);
    const motivation=motivationByFixture?.get(fixture.id);
    if(analysis){
    ...
    const tag=motivation?"a":"article",link=motivation?` href="champions-league.html?match=${esc(fixture.id)}" aria-label="Apri Motivation Index di ${esc(fixture.homeTeam)} - ${esc(fixture.awayTeam)}"`:"";
    return `<${tag}${link} class="reading-fixture match fixture-card${motivation?" fixture-card-link":""} champions-fixture" data-team-home="${esc(fixture.homeTeam)}" data-team-away="${esc(fixture.awayTeam)}" style="--home-color-1:#105ac5;--home-color-2:#052b78;--away-color-1:#1879d5;--away-color-2:#061a57"><header class="match-head"><div class="match-badges"><span class="matchday-chip">Giornata ${fixture.matchday}</span></div><time class="match-date" datetime="${esc(`${fixture.date}T${fixture.kickoff}`)}">${esc(shortDate(fixture.date))} · ${esc(fixture.kickoff)}</time></header><span class="reading-fixture-teams">${teamBadge(fixture.homeTeam,branding)}<b>VS</b>${teamBadge(fixture.awayTeam,branding)}</span>${motivation?`<span class="champions-motivation-preview"><b>${motivation.home.motivation.score}</b><small>Motivation</small><b>${motivation.away.motivation.score}</b></span>`:""}</${tag}>`;
  }

  function fixtureGroups(fixtures,branding,pilotByFixture,motivationByFixture){
  ...
  function motivationOnlyDetail(fixture,entry,branding){
    return `<nav class="reading-back champions-reading-back"><a href="champions-league.html">← Calendario Champions</a><span>Giornata ${fixture.matchday}</span></nav><section class="reading-match-hero champions-reading-match-hero"><div><p class="eyebrow">UEFA Champions League · Motivation Index</p><h1>${esc(fixture.homeTeam)} - ${esc(fixture.awayTeam)}</h1><p>${esc(dayLabel(fixture.date))} · ${esc(fixture.kickoff)}</p></div><div class="reading-matchup champions-reading-matchup">${teamBadge(fixture.homeTeam,branding,{showName:false})}<b>VS</b>${teamBadge(fixture.awayTeam,branding,{showName:false})}</div></section><aside class="reading-prototype-banner champions-reading-warning"><strong>Lettura parziale</strong><p>Per questa gara è disponibile il Motivation Index; proiezioni statistiche complete e formazioni restano N/D.</p></aside>${motivationPanel(entry)}`;
  }

  function motivationOverview(data,branding){
    const objectives=data.objectiveLabels||{};
    const rows=data.teams.map(side=>{const base=side.baseline;return `<tr><th scope="row">${teamBadge(side.team,branding)}</th><td>${esc(base.country)}</td><td data-sort-value="${base.baselineMotivation}"><strong>${base.baselineMotivation}</strong></td><td>${esc(objectives[base.seasonObjective]||base.seasonObjective)}</td><td data-sort-value="${base.clubPressure}">${base.clubPressure}</td><td>${esc(base.experienceLevel)}</td></tr>`}).join("");
  ...
  async function render(){
    ...
    if(requestedCalendarFixture&&motivationByFixture.has(requestedMatchId)){
      document.querySelector("#app").innerHTML=motivationOnlyDetail(requestedCalendarFixture,motivationByFixture.get(requestedMatchId),branding);
      return;
    }
    ...
    const applyFilters=()=>{
      ...
      results.innerHTML=fixtureGroups(filtered,branding,pilotByFixture,motivationByFixture);
