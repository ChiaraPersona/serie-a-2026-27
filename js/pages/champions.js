export function createPage(deps){
  ...
  function championsTeamDetail(team,strength,history,pilot,squads,attackProfile,data,branding){
    ...
    const calendarPanel=`<section class="champions-calendar champions-team-calendar" aria-labelledby="team-calendar-title"><header class="champions-calendar-heading"><div><p class="eyebrow">Fase campionato · ${teamFixtures.length} partite</p><h2 id="team-calendar-title">Calendario di ${esc(team.team)}</h2><p>Orari italiani · ${teamFixtures.filter(item=>item.homeTeam===team.team).length} in casa · ${teamFixtures.filter(item=>item.awayTeam===team.team).length} in trasferta</p></div><a href="${esc(data.source.url)}" target="_blank" rel="noreferrer">Fonte UEFA ↗</a></header>${fixtureGroups(teamFixtures,branding,new Map(pilot.fixtures.map(item=>[item.fixtureId,item])))}</section>`;
