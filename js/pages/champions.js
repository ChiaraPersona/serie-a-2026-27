export function createPage(deps){
  ...
  function pilotReadingDetail(fixture,pilot,backtest,squads,branding,h2h,motivationEntry){
    ...
    const goalForecast=`<article class="prediction-match-volume champions-goal-forecast"><div><p class="eyebrow">Pronostico quantitativo</p><h3>${esc(selection[0])} · ${esc(selection[2])}</h3><small>Probabilità 1/X/2: ${fixture.probabilities.home.toFixed(1)}% · ${fixture.probabilities.draw.toFixed(1)}% · ${fixture.probabilities.away.toFixed(1)}%</small></div><dl><div><dt>Risultato coerente col segno</dt><dd>${esc(fixture.scoreForecast.primary.score)}</dd></div><div><dt>Gol attesi medi</dt><dd>${fixture.expectedGoals.total.toFixed(2)} <small>${fixture.expectedGoals.home.toFixed(2)}–${fixture.expectedGoals.away.toFixed(2)}</small></dd></div><div><dt>Fascia gol probabile</dt><dd>${fixture.goalBand.min}–${fixture.goalBand.max} <small>${fixture.goalBand.probabilityPct.toFixed(1)}%</small></dd></div></dl><p class="champions-goal-explainer">I gol attesi sono la media di tutti gli scenari, non un secondo risultato esatto. ${fixture.scoreForecast.modal.score!==fixture.scoreForecast.primary.score?`La moda assoluta è ${esc(fixture.scoreForecast.modal.score)}, ma appartiene a un esito complessivamente meno probabile del segno ${esc(selection[0])}.`:"Il risultato coerente coincide anche con la moda assoluta."}</p><div class="prediction-probability-list">${probabilityRows(fixture.goals)}</div></article>`;
